 ...
 */
async function analyseAudioAndExtractFeatures(audioFile: Blob): Promise<{
    minFrequency: number,
    maxFrequency: number,
    minIntensity: number,
    maxIntensity: number
...
async function loadAudioFile(filePath: string): Promise<Blob> {
    const response = await fetch(filePath);
    if (!response.ok) throw new Error('Failed to load the audio file');
    return response.blob();
}

// Fonction de test pour analyser le fichier audio et afficher les résultats
async function testAudioAnalysis(filePath: string): Promise<void> {
    try {
        const audioBlob = await loadAudioFile(filePath);
        const features = await analyseAudioAndExtractFeatures(audioBlob);
